...
def render_duel_battle_public(run: GameRun) -> str:
    ...
    lines = [
        ...
        f"进化点: {run.node_data.get('p1_evolve_points', 4)}/4",
        "我方战场：",
    ]
    ...
    if not p1.minions and not p1.amulets:
        ...
        for i in range(1, 7):
            ...
            if key in p1.minions:
                m = p1.minions[key]
                lines.append(f"  [{i}] 随从：{m.name} (❤️ HP {m.hp}/{m.max_hp} | ⚔️ 攻击 {m.atk})")
            elif key in p1.amulets:
                a = p1.amulets[key]
                lines.append(f"  [{i}] 护符：{a.name} (⏳ 吟唱 {a.countdown})")
                
    lines.append("")
    lines.append(f"🔵 玩家二：{p2_name}")
    lines.append(f"❤️ HP {p2.hp}/{p2.max_hp} | 🛡️ 护盾 {p2.shield} | ⚡ 能量 {p2.actions}A {p2.bonus_actions}BA")
    lines.append(f"进化点: {run.node_data.get('p2_evolve_points', 4)}/4")
    lines.append("敌方战场：")
