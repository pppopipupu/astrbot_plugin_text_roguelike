...
class ForgeFireOption(EventOption):
    def execute(self, run, engine) -> str:
        run.node_data["upgrade_source"] = "event"
        return "UPGRADE_FLOW"

class OverloadForgeOption(EventOption):
    def execute(self, run, engine) -> str:
        import random
...
class ForgeBackfireOption(EventOption):
    def execute(self, run, engine) -> str:
        p = run.player
        p.max_hp += 5
        p.hp = min(p.max_hp, p.hp + 10)
...
class ShatterForgeOption(EventOption):
    def execute(self, run, engine) -> str:
        p = run.player
        p.gold += 50
        p.hp -= 6
...
def get_option_by_action(action: str) -> Optional[EventOption]:
    mapping = {
        ...
        "forge_fire": ForgeFireOption("使用常规重铸", "forge_fire"),
        "overload_forge": OverloadForgeOption("过载重铸", "overload_forge"),
        "forge_backfire": ForgeBackfireOption("汲取熔炉余温", "forge_backfire"),
        "shatter_forge": ShatterForgeOption("强行破坏熔炉", "shatter_forge")
    }
