...
- 随从动作机制：在玩家回合交替时，我方随从的 actions 增加 1（即 actions 累加），只有奥术傀儡（arcane_golem）的 bonus_actions 会增加 1，其余随从的 bonus_actions 不变，随从的攻击动作点（AA）每回合重置为 1。非 epic 随从不具备 BA，且未升级技能消耗通常为 2A 或 3A，部分升级版技能的 actions 消耗为 1A。
...
- **可扩展的新机制设计**：
  - 添加新战斗或底层机制（如元素盾击、抗性豁免）时，必须定义新的原子事件（继承自 `GameEvent` 并由 `event_bus` 调度），并对外提供可由 Relic 或 Buff 修改计算数值的标准化字段载荷。
  - 核心处理引擎在进行机制计算时，应只依赖该事件的最终结算数据，以便后续的新卡牌、新被动能够通过单纯订阅该事件完成逻辑扩展，禁止在核心计算流程中加入特定卡牌或状态的 `if` 逻辑。
...
- 本地综合单元测试：在插件根目录下执行：
  ...
  使用标准 `unittest` 框架，依次测试游戏内包括：基础契约与地图流、卡牌保留与回合丢弃、战斗专注、回响形态、局内商店、伤害与真实伤害系统、怪物眩晕与意图同步、三选一与跳过卡牌获取、免前缀模式开关及指令过滤、回合开始护盾流失一半、卡牌升级与熔炉、职业命令别名与子职业选择、等待目标自动取消、临时日志持久化隔离等在内的 14 个核心业务及边界场景。
